...
const configMessages = {
  en: "Configuration error: Please add VITE_DEEPSEEK_API_KEY to your .env.local file",
  fr: "Erreur de configuration : Veuillez ajouter VITE_DEEPSEEK_API_KEY à votre fichier .env.local",
  es: "Error de configuración: Por favor agregue VITE_DEEPSEEK_API_KEY a su archivo .env.local"
};
...
const callDeepSeek = async (apiKey: string, messages: DeepSeekMessage[]): Promise<string> => {
...
export const generateNapoleonResponse = async (history: Message[], lang: AppLanguage): Promise<string> => {
  ...
  if (!apiKey) {
    console.error("CRITICAL ERROR: VITE_DEEPSEEK_API_KEY is missing from environment. Ensure it is defined in .env.local");
...
Ahora genera un desafío DIFERENTE y único. Sé creativo e inspirador.`
  ...
  const messages: DeepSeekMessage[] = [
    { role: 'system', content: "You are Napoleon Hill, master teacher of personal achievement and author of 'Think and Grow Rich'. Generate transformational daily challenges based on your 17 Principles of Success." },
